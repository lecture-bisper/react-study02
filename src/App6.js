...
// 문제 2) state객체에 변수를 추가하여 해당 변수의 값을 매 1초마다 확인하고, 해당 변수의 값이 0이 될때 화면에 출력하는 글자를 We Are Ready!! 로 변경하는 프로그램을 작성하세요
// ps 1) 로딩 중일때는 Loading... x 로 화면에 글자를 출력함, (x 는 숫자)
...
// state 의 값이 변경되었기 때문에 Updating 상태로 변경되어 리액트가 화면을 다시 구성함
...
// 조건이 참이면 state.count -1 을 실행 (state.count가 0이 될때까지)
